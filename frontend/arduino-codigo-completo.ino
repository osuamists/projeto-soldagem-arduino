...
unsigned long ultimoEnvioDados = 0; // NOVO: para enviar dados serialmente
...
void loop() {
  // PRIORIDADE 1: Sempre atualizar o display 7 segmentos (crítico para não piscar)
  atualizarDisplay7Seg();
  
  // PRIORIDADE 3: Resto da lógica
  lerSensores();
  gerenciarSeguranca();
  prepararDados7Seg();
  tratarBotoes();
  ...
  // PRIORIDADE 2: Envia JSON a cada 500ms para o webserver ler
  if(millis() - ultimoEnvioDados > 500) {
    // Formato JSON estruturado em UMA ÚNICA LINHA
    Serial.print("{\"tensao\":");Serial.print(tensao);
    Serial.print(",\"corrente\":");Serial.print(corrente);
    Serial.print(",\"temperatura\":");Serial.print(temperatura);
    Serial.print(",\"fluxo\":");Serial.print(fluxo);
    Serial.print(",\"rpm\":");Serial.print(rpm);
    Serial.print(",\"modo\":");Serial.print(relesModoSMAW ? 1 : 0);
    Serial.print(",\"sistema\":");Serial.print(relesSistemaLigado ? 1 : 0);
    Serial.print(",\"alarme\":");Serial.print(alarmeGlobalAtivo ? 1 : 0);
    Serial.println("}"); // FECHA O JSON NA MESMA PRINTLN
    ultimoEnvioDados = millis();
  }
}
